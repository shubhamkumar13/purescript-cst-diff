...
main = do
    ...
    case src of
        ...
        Right b ->  
            ...
            IO.writeFile "something.txt" (show $ toJSObject (map f b)) >> 
            Prelude.putStrLn ""
...
f :: CST.SourceToken -> (String, ((String, (String, [(String, (String, [(String, String)]))])), (String, [String])))
f CST.SourceToken{..} = 
    -- (show tokAnn, show tokValue)
    ("SourceToken", (g1 tokAnn, g2 tokValue))
g1 CST.TokenAnn{..} = 
...
h CST.SourceRange{..} = ("SourceRange", [("srcStart", k srcStart), ("srcEnd", k srcEnd)])

k CST.SourcePos{..} = ("SourcePos", [("srcLine", show srcLine), ("srcColumn", show srcColumn)])